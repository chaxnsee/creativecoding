export class FaceProcessor {
  ...
  async process(source, smoothing) {
    ...
    if (now - this.lastProcess < 78) return this.lastResult;
  ...
  depthAt(nx, ny, mirrored = false) {
    ...
    for (const index of FACE_DEPTH_INDICES) {
    ...
    let depth = depthFromNearest(z, nearest, result.bounds, 5.8, 1.28);
  ...
  handDepthAt(x, y, hand) {
    ...
    for (const index of HAND_DEPTH_INDICES) {
    ...
    return depthFromNearest(z, nearest, hand.bounds, 6.8, 1.08);
  }
}

function depthFromNearest(z, nearest, bounds, falloff, boost) {
  const range = Math.max(0.0001, bounds.maxZ - bounds.minZ);
  const close = Math.pow(1 - (z - bounds.minZ) / range, 0.72);
  const influence = Math.max(0, 1 - Math.sqrt(nearest) * falloff);
  return clamp(close * influence * boost, 0, 1);
}
