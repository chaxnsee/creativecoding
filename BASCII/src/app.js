...
import { BurmeseAsciiRenderer } from "./renderer.js?v=hands-depth-pretext-20260502";
...
const elements = {
  ...
  pretextButton: document.querySelector("#pretextButton"),
  pretextText: document.querySelector("#pretextText"),
...
const UI_COPY = {
  en: {
    ...
    rain: "Rain",
    orbit: "Orbit",
    pretextSession: "Pretext Session",
    cameraOnly: "Camera only",
    pretextMotion: "Motion Text",
    pretextText: "Text",
    ...
    pretextActive: "Pretext-inspired motion text is active. Credit: Cheng Lou.",
...
function bindControls() {
  ...
  elements.pretextButton.addEventListener("click", () => {
    state.pretextMode = !state.pretextMode;
    elements.pretextButton.classList.toggle("active", state.pretextMode);
    elements.pretextButton.setAttribute("aria-pressed", String(state.pretextMode));
    if (state.pretextMode) setStatus("pretextActive");
  });
  ...
  elements.txtButton.addEventListener("click", () => exporter.saveText());
  elements.copyButton.addEventListener("click", () => exporter.copyText());
...
function syncState() {
  ...
  state.pretextText = elements.pretextText.value;
...
function syncTextExport() {
  const now = performance.now();
  if (now - lastTextExportAt < 300 || document.activeElement === elements.textExport) return;
  lastTextExportAt = now;
...
function exitLoveModeForAmbient() {
  if (!elements.loveButton.classList.contains("active")) return;
  ...
  elements.loveButton.classList.remove("active");
  elements.flowStrength.value = "0.75";
  applyPreset("terminal");
}
